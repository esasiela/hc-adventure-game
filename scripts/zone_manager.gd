...
func register_minimap(m: Node) -> void:
	minimap = m
...
func _apply_camera_bounds() -> void:
	...
	var player_camera: Camera2D = player.get_node("Camera2D")
	if player_camera:
		_set_camera_limits(player_camera, bounds)
	
	var minimap_camera: Camera2D = minimap.get_camera()
	if minimap_camera:
		_set_camera_limits(minimap_camera, bounds)


func _set_camera_limits(camera: Camera2D, bounds: Rect2) -> void:
